...
float jp = -6;
float vd = 0;
float pc = -0.5; //-0.5 for 113 -0.8 for 109
float pt = 0;
float theta_d = 0;
float vl_error = 0;
float vr_error = 0;
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float delta_t) {
  ...
  pt += (vL+vR)/2 *delta_t; //our position, defined by the linear velocity times time
  error_theta = pc*pt-angleRad; //the error in theta, which is defined by our position minus our current angle, times a position controller, pc

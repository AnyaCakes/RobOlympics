// This code allows Rocky to perform the sprint challenge
...
float ki = 5000;
float error_theta = 0;
float error_theta_accum = 0;
float ji = -36;
float jp = -6;
float vd = 0;
float pc = -0.8; //-0.5 for 113 -0.8 for 109
float vt = 0;
float theta_d = 0;
float vl_error = 0;
float vr_error = 0;
float vl_error_accum = 0;
float vr_error_accum = 0;
float t = 0;
...
void updatePWMs(float totalDistanceLeft, float totalDistanceRight, float vL, float vR, float angleRad, float angleRadAccum, float delta_t) {
   ...
   */
  t += delta_t; //accumulated time
  //theta_d = 0.02*sin(t)-0.02;
  vt += (vL+vR)/2 *delta_t; //our position, defined by the linear velocity times time
  //the error in theta, which is defined by our position minus our current angle, times a position controller, pc
  //we then add t/3.85 to our position, so that Rocky expects to have gone an additional t/3.85 distance
  error_theta = pc*(vt+t/3.85)-angleRad; 
  error_theta_accum += error_theta*delta_t; //our total error accumulated
  vd=jp*error_theta + ji*error_theta_accum; //our first controller, which gives us velocity desired based off of error calculated earlier, and two controller constants, ji and jp
  //calculate error from desired velocity and current velocity
  vl_error = vd - vL;
  vr_error = vd - vR;
  //calculate accumulated velocity error
  vl_error_accum += vl_error*delta_t;
  vr_error_accum += vr_error*delta_t;
  //last controller, which is fed into PWM. Uses kp and ki
  leftMotorPWM = kp*vl_error  + ki*vl_error_accum ;
  rightMotorPWM = kp*vr_error + ki*vr_error_accum ;
